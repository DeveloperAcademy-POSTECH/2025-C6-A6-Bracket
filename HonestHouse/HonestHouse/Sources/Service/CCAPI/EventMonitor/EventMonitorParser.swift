...
actor EventMonitorParser {
    ...
    private func parseNextEvent() -> CameraStatus.EventMonitorResponse? {
        ...
        Logger.debug("parseNextEvent: buffer size = \(localBuffer.count) bytes", category: .network)
        ...
        Logger.debug("Checking Start Byte: [0]=0x\(String(format: "%02X", localBuffer[0])) [1]=0x\(String(format: "%02X", localBuffer[1]))", category: .network)
        ...
        Logger.debug("Checking Data Type: [2]=0x\(String(format: "%02X", localBuffer[2]))", category: .network)
        guard localBuffer[2] == 0x02 else {
            Logger.warning("Invalid Data Type (expected 0x02)", category: .network)
            self.buffer.removeFirst(3)
            return nil
        }
        Logger.debug("Data Type OK (Event Data)", category: .network)
        ...
        guard localBuffer.count >= totalSize else {
            Logger.debug("Buffer too small (need \(totalSize), have \(localBuffer.count)), waiting for more data", category: .network)
            return nil
        }
        ...
        Logger.debug("Checking End Byte at index \(endByteIndex): [0x\(String(format: "%02X", localBuffer[endByteIndex]))] [0x\(String(format: "%02X", localBuffer[endByteIndex + 1]))]", category: .network)
        ...
        Logger.debug("Extracting event data: \(jsonData.count) bytes", category: .network)

        // 처리한 데이터 제거 (self.buffer만 수정)
        self.buffer.removeFirst(totalSize)
        Logger.debug("Removed \(totalSize) bytes from buffer, remaining: \(self.buffer.count) bytes", category: .network)

        // JSON 디코딩
        do {
            let decoder = JSONDecoder()
            let event = try decoder.decode(CameraStatus.EventMonitorResponse.self, from: jsonData)
            Logger.debug("Event decoded successfully", category: .network)
            return event
        } catch {
            Logger.error("EventMonitor JSON decode error: \(error)", category: .network)
            return nil
        }
    }
}
